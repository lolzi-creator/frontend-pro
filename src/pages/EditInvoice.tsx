...
const EditInvoice: React.FC = () => {
  ...
  const loadInvoice = async (invoiceId: string) => {
    try {
      ...
      if (response.success && response.data.invoice) {
        ...
        // Convert items to match our format
        const items = (invoice.items || invoice.lineItems || []).map((item: any) => ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) {
      showToast({ type: 'error', title: t.invoice.failedToUpdateInvoice || 'Invalid invoice ID' })
      return
    }
    ...
    try {
      ...
      const response = await apiClient.updateInvoice(id, invoiceData)
      
      if (response.success) {
        showToast({ 
          type: 'success', 
          title: t.invoice.invoiceUpdatedSuccess || 'Invoice updated successfully!'
        })
        
        // Navigate back to invoice detail page
        navigate(`/invoices/${id}`)
      } else {
        showToast({ type: 'error', title: t.invoice.failedToUpdateInvoice || 'Failed to update invoice' })
      }
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">
            {loadingInvoice ? (t.invoice.loadingInvoice || 'Loading invoice...') : (t.invoice.loadingCustomers || 'Loading customers...')}
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">{t.invoice.edit}</h1>
          <p className="mt-2 text-gray-600">{t.invoice.editSubtitle || 'Update invoice details below'}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-end space-x-4">
            ...
            <Button
              ...
              disabled={submitting}
            >
              {submitting ? (t.invoice.updatingInvoice || 'Updating Invoice...') : t.common.save}
            </Button>
          </div>
        </form>
      </div>
