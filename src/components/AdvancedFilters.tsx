    ...
    switch (filter.type) {
      case 'select':
        return (
          ...
          >
            <option value="">{t.common.all || 'All'} {filter.label}</option>
      ...
      case 'text':
        return (
          <input
            ...
            value={value}
            onChange={(e) => handleFilterChange(filter.key, e.target.value)}
            placeholder={filter.placeholder || `${t.common.filterBy || 'Filter by'} ${filter.label}`}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
          />
        )
  ...
  return (
    <div className={className}>
      ...
      {isOpen && (
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>{t.common.filterOptions || 'Filter Options'}</CardTitle>
          </CardHeader>
          <CardContent>
